...
function PLAYER:LegAttack()
    if self:GetNWBool("zb_noclip_meaty", false) then return end
	...
	if self:HasWeapon("weapon_slasherhands_sh") then
		handClass = "weapon_slasherhands_sh"
	else
...
concommand.Add("hg_kick", function(ply)
    if ply:GetNWBool("zb_noclip_meaty", false) then return end
