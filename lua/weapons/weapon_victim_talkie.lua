...
SWEP.Instructions = "Use the walkie-talkie to communicate with other people in the 4km radius. there's only one frequency."
...
if not IsValid(inputRadio) then
    return false
end

if not self:GetIsOn() or not inputRadio:GetIsOn() then
    return false
end
...
function SWEP:PrimaryAttack()
    ...
    tbl[#tbl + 1] = {
        ...
        self:GetIsOn() and "Turn off Walkie-Talkie" or "Turn on Walkie-Talkie"
    }

    hg.CreateRadialMenu(tbl)
end

function SWEP:AdjustFrequency()
